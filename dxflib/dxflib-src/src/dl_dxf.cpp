...
bool DL_Dxf::handleLWPolylineData(DL_CreationInterface* /*creationInterface*/) {
    ...
    if (groupCode==90) {
        ...
        if (maxVertices>0) {
            ...
            vertices = new double[9*maxVertices];
            for (int i=0; i<maxVertices; ++i) {
                vertices[i*9] = 0.0;
                vertices[i*9+1] = 0.0;
                vertices[i*9+2] = 0.0;
    ...
    // Compute LWPolylines vertices (group codes 10/20/30/42):
    else if (groupCode==74 || groupCode==71 ||
             groupCode==72 || groupCode==73) {
        ...
        if (groupCode<=30) {
            if (vertexIndex>=0 && vertexIndex<maxVertices) {
                vertices[9*vertexIndex + (groupCode/10-1)]
                = toReal(groupValue);
            }
        } else if (groupCode==42 && vertexIndex<maxVertices) {
            vertices[9*vertexIndex + 3] = toReal(groupValue);
        }
        return true;
    }
    switch(groupCode)
    {
    case 70:
        vertices[9*vertexIndex + 4] = toInt(groupValue);
        return true;
    case 71:
        vertices[9*vertexIndex + 5] = toInt(groupValue);
        return true;
    case 72:
        vertices[9*vertexIndex + 6] = toInt(groupValue);
        return true;
    case 73:
        vertices[9*vertexIndex + 7] = toInt(groupValue);
        return true;
    case 74:
        vertices[9*vertexIndex + 8] = toInt(groupValue);
        return true;
    default:
      return false;
    }
...
void DL_Dxf::add3dFace(DL_CreationInterface* creationInterface) {
	...
	DL_3dFaceData da(
        ...
        toReal(values[22], 0.0),
        toReal(values[32], 0.0),
        // definition point 4
        toReal(values[13], 0.0),
        toReal(values[23], 0.0),
        toReal(values[33], 0.0));
    creationInterface->add3dFace(da);
}
