...
)
{
    ...
    tree = pointer_to_tree;//(Tree *) ((long) pointer_to_tree[0]);
    
    //trpr[0]=1.0;
    //trpr[4]=1.0;
    //trpr[8]=1.0;

		printf("Applied initial Rotation %f %f %f \n", trpr[0], trpr[1],trpr[2]);
		printf("Applied initial Rotation %f %f %f \n", trpr[3], trpr[4],trpr[5]);
		printf("Applied initial Rotation %f %f %f \n", trpr[6], trpr[7],trpr[8]);
		printf("Applied initial Translations %f %f %f \n\n", ttpr[0], ttpr[1],ttpr[2]);
    
    //if((nrandz+1)>ndataz){
    //    bol=0;
    //}
    printf("Starting ICP main loop \n"); 
	...
	//reset output HTuples to obtain latest matching
	if(ii==iimax-1)
	{
		*model_indices = Halcon::HTuple();
		*data_indices = Halcon::HTuple();
	}
		...
		//write Indizes to data output
		if(ii==iimax-1)
		{
			Halcon::tuple_concat(*model_indices, (int)k, model_indices);
			Halcon::tuple_concat(*data_indices, (int)i, data_indices);
		}		
...
printf("Applied final Rotation %f %f %f \n", trpr[0], trpr[1],trpr[2]);
printf("Applied final Rotation %f %f %f \n", trpr[3], trpr[4],trpr[5]);
printf("Applied final Rotation %f %f %f \n", trpr[6], trpr[7],trpr[8]);
printf("Applied final Translations %f %f %f \n\n", ttpr[0], ttpr[1],ttpr[2]);
...
//        mexErrMsgTxt("Dimension of model points must be 3.");
//    }
//    else if (mxGetM(data)!=3){
//        mexErrMsgTxt("Dimension of data points must be 3.");
//    }
//    else if ((mxGetM(weights) != mxGetN(data)) && (mxGetN(weights) != mxGetN(data))){
//        mexErrMsgTxt("Number of weights must be equal to the number of data points.");
//    }
//    else if ((mxGetM(randvec) != mxGetN(data)) && (mxGetN(randvec) != mxGetN(data))){
//        mexErrMsgTxt("Length of randvec must be equal to the number of data points.");
//    }
